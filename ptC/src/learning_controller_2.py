...
#class used for finding the union, based on networkx UnionFind
class UnionStruct:
    def __init__(self):
      self.weights = {}
      self.parents = {}
    ...
    def union(self, *objects):
      ...
      maxWeight = max([(self.weights[r],r) for r in roots])[1]
      for r in roots:
        if r != maxWeight:
          self.weights[maxWeight] += self.weights[r]
          self.parents[r] = maxWeight

#function to compute min spanning edges of a networkx graph
def get_min_edges(G):
    subtrees = UnionStruct()
    edges = G.edges()
    res = []
    for u, v in edges:
